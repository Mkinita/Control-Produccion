...
import ListadoProduccionFechaAsr from '../components/ListadoProduccionFechaAsr'
...
export default function AdminProducciones() {
    ...
    const fetcher = () => axios('/api/listado-produccion-fecha-asr').then(datos => datos.data)
    const { data, error, isLoading } = useSWR('/api/listado-produccion-fecha-asr',fetcher,{refreshInterval: 100} )
  ...
  const [ search, setSearch ] = useState("")
    ...
    const URL = '/api/listado-produccion-fecha-asr'
  ...
  const sumarVolumenes = () => {
    ...
    return(
        <LayoutAsr pagina={'Produccion-fecha'}>
            ...
            <tbody>
                    <tr className="bg-white border-b text-sm">
                        ...
                        <td className="px-6 py-4 w-1/5 text-center border border-amber-400">Espesor</td>
                        <td className="px-6 py-4 w-1/5 text-center border border-amber-400">Ancho</td>
                        <td className="px-6 py-4 w-1/5 text-center border border-amber-400">Largo</td>
                        <td className="px-6 py-4 w-1/5 text-center border border-amber-400">m³</td>
