...
export default function AdminProducciones() {
  ...
  const [users5, setUsers5] = useState([]);
    ...
    showData5();
        ...
        const horasMes = users5.map(user => parseInt(user.horasmesseco, 10));
        const horasTrabajadas = users5.map(user => parseInt(user.horastrabajadasseco, 10));
        ...
        <table className="table-auto w-full text-center bg-white text-gray-700 font-bold">
                <tbody>
                        ...
                        <tr className="bg-white">
                            <td className="px-1 py-4 w-1/3 text-center border border-lime-400">Fecha</td>
                            <td className="px-1 py-4 w-1/3 text-center border border-lime-400">ING.</td>
                            <td className="px-1 py-4 w-1/3 text-center border border-lime-400">m³/hr</td>
                        </tr>
          ...
          <table className="table-auto w-full text-center bg-white">
            <tbody>
                    ...
                    <tr className="bg-white text-sm uppercase font-bold">
                        <td className="px-2 py-4 w-1/3 text-center">Total</td>
                        <td className="px-2 py-4 w-1/3 text-center">{formatoNumero(totalIngreso)}</td>
                        <td className="px-2 py-4 w-1/3 text-center">{formatoNumero(totalIngreso / TOTAL)}</td>
